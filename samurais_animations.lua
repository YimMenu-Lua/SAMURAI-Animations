    ...
    if ImGui.Button("Stop") then
        if info ~= nil and is_playing_anim then
            script.run_in_fiber(function(script)
                ...
                ENTITY.SET_ENTITY_COORDS_NO_OFFSET(ped, current_coords.x, current_coords.y, 0, true, false, false)
                script:sleep(3000)
                if STREAMING.HAS_ANIM_DICT_LOADED(info.dict) then gui.show_warning("FU%CK!!!") end
